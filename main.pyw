...
def center(width, height):
    screen_width, screen_height = Info(scaled=False)
    scale = Info(False)[0] / tk.winfo_screenwidth()
    return f"{width}x{height}+{int(screen_width / 2 - width * scale / 2)}+{int(screen_height / 2 - height * scale / 2)}"

tk.rowconfigure(0, weight=1)
tk.columnconfigure(0, weight=0)
tk.columnconfigure(1, weight=1)
width, height = 1024, 512
tk.geometry(center(width, height))
...
class Orchestrator:
    ...
    def __init__(self):
        ...
        self.wish_var = StringVar(tk, value="[]")
    ...
    def init_logging_window(self):  # logger requires: name, status, log
        ...
        tl.geometry(f"{width}x{height}+{0}+{0}")
        tk.tkraise(tl)

        li_var = StringVar(value="[]")
        li_items = []
        cur_select = None
        ...
        li = CTkListbox(tl, listvariable=li_var)
        ...
        def render_worker():
            ...
            while True:
                ...
                if item is None:  # render list
                    ...
                    items = [f'{x.name} [{x.status}]' for x in li_items]
                    li_var.set(repr(items)) 
    ...
    def init_main_window(self):
        btnpanel = ctk.CTkFrame(tk)
        btnpanel.grid(row=0, column=0, sticky='nsew')
        btnpanel.grid_rowconfigure(5, weight=1)
        ...
        ctk.CTkSwitch(btnpanel, text='Captcha', variable=auto_captcha_var,
                        onvalue='on', offvalue='off').grid(row=2, column=0, padx=10, pady=5, sticky="ew")
        ctk.CTkButton(btnpanel, text='Refresh',
                      command=self.refresh).grid(row=3, column=0, padx=10, pady=5, sticky="ew")
        ctk.CTkSwitch(btnpanel, text='Auto', command=ref_changed,
                        variable=auto_refresh_var, onvalue='on', offvalue='off').grid(row=4, column=0, padx=10, pady=5, sticky="ew")
        ...
        ctk.CTkLabel(btnpanel, text='Intv:').grid(row=6, column=0, padx=10, pady=5, sticky="ew")
        interval_entry = ctk.CTkEntry(
            btnpanel, textvariable=interval_var)
        interval_entry.bind('<Return>', change_time_config)
        interval_entry.grid(row=7, column=0, padx=20, pady=5, sticky="ew")
        ctk.CTkLabel(btnpanel, text='Timeout:').grid(row=8, column=0, padx=10, pady=5, sticky="ew")
        timeout_entry = ctk.CTkEntry(
            btnpanel, textvariable=timeout_var)
        timeout_entry.bind('<Return>', change_time_config)
        timeout_entry.grid(row=9, column=0, padx=20, pady=5, sticky="ew")

        def verbose_changed():
            Logger.VERBOSE = verbose_var.get() == 'on'
            self.log('info', f'verbose changed to {Logger.VERBOSE}')

        ctk.CTkSwitch(btnpanel, text='Verbose', command=verbose_changed,
                        variable=verbose_var, onvalue='on', offvalue='off').grid(row=10, column=0)
        ...
        wish_box.grid(row=0, column=2, sticky='ns')
    ...
    def update_wish_var(self):
        ...
        self.wish_var.set(repr(display_items))
